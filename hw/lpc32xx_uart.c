...
#define omap_uart_attach  omap_uart_attach__hidden
...
#define omap_uart_read  omap_uart_read__proper
...
#undef omap_uart_read

static uint64_t omap_uart_read(void *opaque, target_phys_addr_t addr,
                               unsigned size)
{
    uint64_t value = omap_uart_read__proper(opaque, addr, size);
    struct omap_uart_s *s = (struct omap_uart_s *) opaque;

if(01)
    fprintf(stderr, "%s.r +%02lx[%ld] %8lx\n",
            s->busdev.qdev.id,
            (long)addr, (long)size, (long)value);
...
if(01)
    fprintf(stderr, "%s.w +%02lx[%ld] %08lx\n",
            s->busdev.qdev.id,
            (long)addr, (long)size, (long)value);
...
static void (*rtfp) (void) = omap_uart_register_types;


static TypeInfo lpc32xx_uart_info = {
    .name = "lpc32xx-uart",
    .parent = TYPE_SYS_BUS_DEVICE,
    .instance_size = sizeof(struct omap_uart_s),
    .class_init = omap_uart_class_init,
...
static void lpc32xx_uart_register_types(void)
{
    rtfp = rtfp;
    type_register_static(&lpc32xx_uart_info);
}

type_init(lpc32xx_uart_register_types)
